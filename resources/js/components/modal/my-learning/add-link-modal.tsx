...
export default function AddLinkModal({ title, placeholder, action, id, onClose, onSuccess }: AddLinkModalProps) {
    ...
    return (
        <div className="fixed inset-0 z-99 flex items-center justify-center bg-[#3ABEFF]/40 backdrop-blur-sm">
            <div className="relative w-[400px] rounded-2xl bg-white dark:bg-[#222831] p-6 text-center shadow-md dark:shadow-white/20">
                <button onClick={onClose} className="absolute top-3 right-3 text-gray-500 hover:text-gray-700">
                    <X size={18} />
                </button>

                <h2 className="mb-2 text-lg font-semibold text-gray-800 dark:text-white">{title}</h2>
                ...
                <Form onSuccess={() => onSuccess()} action={route(action, id)} method="post" className="text-left">
                    <input
                        ...
                        name="link"
                        className="w-full rounded-lg border dark:border-white/20 px-4 py-2 text-sm focus:ring-2 focus:ring-[#3ABEFF] focus:outline-none dark:text-white shadow-sm dark:shadow-white/20"
                    />
