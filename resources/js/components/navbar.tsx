...
export default function Navbar() {
    ...
    return (
        <>
            <nav
                className={`fixed top-0 z-50 w-full bg-white shadow-sm transition-transform duration-300 ease-in-out dark:bg-[#222831] dark:shadow-white/20 ${showNavbar ? 'translate-y-0' : '-translate-y-full'}`}
            >
                <div className="relative mx-auto flex items-center justify-between px-6 py-4">
                    <div className="flex cursor-pointer items-center" onClick={() => router.get(route('home'))}>
                        <svg
                            width="64"
                            height="36"
                            viewBox="0 0 78 44"
                    ...
                    <div className="absolute top-1/2 left-1/2 hidden -translate-x-1/2 -translate-y-1/2 items-center space-x-8 font-medium text-[#3ABEFF] lg:flex">
                        <span onClick={() => router.get(route('home'))} className="cursor-pointer hover:text-[#1AAAE3]">
                            Home
                        </span>
                        <span onClick={() => router.get(route('list-course', { category_id: 1 }))} className="cursor-pointer hover:text-[#1AAAE3]">
                            Course
                        </span>
                        <span onClick={() => router.get(route('dashboard'))} className="cursor-pointer hover:text-[#1AAAE3]">
                            MyDashboard
                        </span>
                    </div>
            ...
            <div className="fixed right-0 bottom-0 left-0 z-50 flex justify-around border-t bg-white py-3 shadow-lg lg:hidden dark:border-white/30 dark:bg-[#222831]">
