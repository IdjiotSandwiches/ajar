  ...
  return (
    <section className="pt-8 pb-16 px-6 md:px-12">
      <div className="flex items-center justify-between mb-8">
        ...
        <button className="text-[#42C2FF] font-medium hover:underline text-sm">
          View All →
        </button>
      </div>

      <div className="flex gap-8 overflow-x-auto pb-4 snap-x snap-mandatory scrollbar-thin scrollbar-thumb-[#42C2FF]/30 scrollbar-track-transparent scroll-smooth">
        {dummyInstitutions.map((inst, index) => (
          <div
            key={index}
            className="snap-start flex-shrink-0 w-[120px] flex flex-col items-center"
          >
            <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-[#42C2FF]/20 shadow-sm bg-gray-100 flex items-center justify-center">
              <img
                src={inst.logo}
                alt={inst.name}
