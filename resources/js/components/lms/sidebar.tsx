...
export default function Sidebar({
    ...
    return (
        <>
            <div
                className={`fixed inset-0 z-40 md:hidden bg-black/40 transition-opacity duration-300 ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
                onClick={onClose}
            />
            <aside className={`h-full flex flex-col bg-white border-r z-50 transition-all duration-300 ${collapsed ? "md:w-20" : "md:w-64"} fixed inset-y-0 left-0 md:static transform transition-transform duration-300 ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"} w-64`}>
                <button
                    onClick={onClose}
                    className={`md:hidden absolute top-6 right-4 bg-[#3ABEFF]/10 hover:bg-[#3ABEFF]/20 p-2 rounded-full shadow text-[#3ABEFF] transition-all duration-300 ${mobileOpen ? "opacity-100" : "opacity-0"}`}
                >
                    <X size={18} />
                </button>
                            ...
                            <li key={m.id} className={`${collapsed ? "flex justify-center" : ""}`}>
                                <button
                                    ...
                                    className={`flex items-center rounded-xl transition-all duration-300
                                        ...
                                        ${collapsed
                                            ? "justify-center px-0 w-10 h-10 gap-0"
                                            : "justify-start px-4 py-3 w-full h-10 gap-3"
                                        }
                                    `}
                                >
                                    <span>{m.icon}</span>
