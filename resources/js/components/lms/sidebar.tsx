...
import { Album, BookA, BookCheck, ChevronDown, ChevronLeft, ChevronRight, LogOut, X } from 'lucide-react';
...
const MENU_STUDENT: NavItem[] = [
    ...
    { id: 'mycourses', label: 'My Courses', icon: <Album size={18} />, route: 'my-courses' },
...
export default function Sidebar({ collapsed, mobileOpen, onClose, onToggleCollapse }: any) {
    ...
    return (
        <>
            <div
                className={`fixed inset-0 z-40 bg-[#3ABEFF]/40 backdrop-blur-sm transition-opacity md:hidden dark:bg-gray-700/40 ${mobileOpen ? 'pointer-events-auto opacity-100' : 'pointer-events-none opacity-0'}`}
                onClick={onClose}
            />

            <aside
                className={`fixed inset-y-0 left-0 z-50 flex h-full flex-col bg-white transition-all duration-300 md:static md:bg-transparent dark:bg-[#222831] dark:md:bg-transparent ${collapsed ? 'md:w-20' : 'md:w-64'} ${mobileOpen ? 'translate-x-0' : '-translate-x-full md:translate-x-0'} `}
            >
                <div className="my-4">
                    <div className={`flex h-16 items-center ${!collapsed ? 'justify-between' : 'justify-center'} mx-4 border-b dark:border-white/20`}>
                        <div className="flex cursor-pointer items-center gap-2" onClick={() => router.get(route('home'))}>
                            {!collapsed && (
                        ...
                        <button onClick={onClose} className="rounded-full bg-[#3ABEFF]/10 p-2 text-[#3ABEFF] md:hidden">
                            <X size={18} />
                        </button>
                                ...
                                <li key={item.id}>
                                    <button
                                        ...
                                        className={`group flex items-center rounded-xl transition-all ${
                                            isParentActive
                                                ? 'bg-[#EEF6FF] text-[#3ABEFF] dark:bg-gray-700'
                                                : 'text-gray-600 hover:bg-gray-100 dark:text-white/70 dark:hover:bg-white/20'
                ...
                <div className={`mx-4 flex items-center gap-3 py-4 ${collapsed ? 'justify-center' : ''} border-t dark:border-white/20`}>
                    <img src={storageUrl(user?.profile_picture)} alt="profile" className="h-10 w-10 rounded-full object-cover" />

                    {!collapsed && (
                        <>
                            <div className="flex flex-1 flex-col">
                                <span className="text-sm font-semibold">{user?.name}</span>
                                <span className="text-xs text-gray-500 capitalize dark:text-white/50">
                                    {Object.keys(roles).find((key) => roles[key] === user?.role_id) ?? 'User'}
                                </span>
                            </div>

                            <button onClick={() => router.post(route('logout'))} className="cursor-pointer text-red-500 hover:underline">
                                <LogOut size={20} />
                            </button>
                        </>
                    )}
