...
export default function Sidebar({
    ...
    return (
        <>
            <div
                className={`fixed inset-0 z-40 md:hidden bg-black/40 transition-opacity ${mobileOpen ? "opacity-100 pointer-events-auto" : "opacity-0 pointer-events-none"}`}
                onClick={onClose}
            />

            <aside className={`fixed inset-y-0 left-0 z-50 flex h-full flex-col border-r bg-white transition-all duration-300 md:static w-64 ${collapsed ? "md:w-20" : "md:w-64"} ${mobileOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"}`}>
                <button
                    onClick={onClose}
                    className="md:hidden absolute top-6 right-4 rounded-full bg-[#3ABEFF]/10 p-2 text-[#3ABEFF]"
                >
                    <X size={18} />
                </button>
                                ...
                                <li key={item.id}>
                                    <button
                                        ...
                                        className={`flex items-center rounded-xl transition-all
                                            ${isParentActive ? "bg-[#3ABEFF] text-white" : "hover:bg-[#3ABEFF]/10"}
                                            ${collapsed
                                                ? "justify-center w-10 h-10 mx-auto"
                                                : "justify-between px-4 py-3 w-full"
                                            }
                                        `}
                                    >
